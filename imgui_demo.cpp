...
static void ShowDemoWindowMisc()
{
    ...
    if (ImGui::CollapsingHeader("Inputs, Navigation & Focus"))
    {
        ...
        if (ImGui::TreeNode("Dragging"))
        {
            ...
            if (ImGui::IsItemActive())
                ImGui::GetForegroundDrawList()->AddLine(io.MouseClickedPos[0], io.MousePos, ImGui::GetColorU32(ImGuiCol_Highlight), 4.0f); // Draw a line between the button and the mouse cursor
...
bool ImGui::ShowStyleSelector(const char* label)
{
    ...
    if (ImGui::Combo(label, &style_idx, "Gray\0"))
    {
        switch (style_idx)
        {
        case 0: ImGui::StyleColorsGray(); break;
        }
        return true;
    }
    return false;
